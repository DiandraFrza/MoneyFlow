...
const applySheetStyling = (sheet: XLSX.WorkSheet, headerFill: string, rowFill: string, accentFill: string) => {
  ...
  for (let row = range.s.r; row <= range.e.r; row += 1) {
    for (let col = range.s.c; col <= range.e.c; col += 1) {
      ...
      cell.s = {
        fill: {
          fgColor: { rgb: isHeader ? headerFill : row % 2 === 0 ? rowFill : "FFFFFF" },
          patternType: "solid",
        },
        font: {
          bold: isHeader,
          color: { rgb: isHeader ? "FFFFFF" : "111827" },
          sz: isHeader ? 11 : 10,
        },
        border: {
      ...
      } else if (accentFill && col === 5 && typeof cell.v === "number") {
        cell.s = {
          ...cell.s,
          fill: { fgColor: { rgb: accentFill }, patternType: "solid" },
        };
      }
    }
  }
...
export const exportToExcel = (data: ExportData) => {
  ...
  applySheetStyling(summarySheet, "2563EB", "F8FAFC", "DBEAFE");
  ...
  const transactionRows = data.transactions.map((t) => ({
    ID: t.id,
    Tanggal: t.date,
    Tipe: t.type === "income" ? "Pemasukan" : t.type === "expense" ? "Pengeluaran" : t.type === "savings" ? "Tabungan" : t.type === "debt_payment" ? "Bayar Utang" : "Cicilan",
  ...
  applySheetStyling(txSheet, "4F46E5", "F8FAFC", "E0E7FF");
  ...
  applySheetStyling(walletSheet, "10B981", "F0FDF4", "DCFCE7");
  XLSX.utils.book_append_sheet(wb, walletSheet, "Dompet");

  // D. Budgets Sheet
  const budgetRows = data.budgets.map((b) => {
    // calculate spent
    const spent = data.transactions.filter((t) => t.category_id === b.category_id && (t.type === "expense" || t.type === "installment")).reduce((sum, t) => sum + t.amount, 0);
    return {
  ...
  applySheetStyling(budgetSheet, "F59E0B", "FFFBEB", "FEF3C7");
  ...
  applySheetStyling(savingsSheet, "0EA5E9", "F0F9FF", "E0F2FE");
  ...
  applySheetStyling(debtSheet, "8B5CF6", "FAF5FF", "EDE9FE");
...
export const exportToPDF = (data: ExportData, healthScore: number, healthStatus: string) => {
  ...
  const walletBody = data.wallets.map((w) => [w.name, w.type.toUpperCase(), formatCurrencyStr(w.balance)]);
  ...
  const budgetBody = data.budgets.map((b) => {
    ...
    return [categoryMap.get(b.category_id) || "Lainnya", formatCurrencyStr(b.amount), formatCurrencyStr(spent), formatCurrencyStr(balance), isOver ? "OVER BUDGET" : "OK"];
  });
  ...
  const savingsBody = data.savings.map((s) => [s.name, formatCurrencyStr(s.target_amount), formatCurrencyStr(s.current_amount), formatCurrencyStr(Math.max(0, s.target_amount - s.current_amount)), `${Math.round((s.current_amount / s.target_amount) * 100)}%`]);
  ...
  const debtBody = data.debts.map((d) => [d.person_name, d.type === "borrowed" ? "HUTANG (Kita Pinjam)" : "PIUTANG (Kita Pinjamkan)", formatCurrencyStr(d.amount), formatCurrencyStr(d.remaining_amount), d.due_date || "-", d.status.toUpperCase()]);
  ...
  const txBody = recentTransactions.map((t) => [t.date, t.type === "income" ? "Pemasukan" : t.type === "expense" ? "Pengeluaran" : t.type === "savings" ? "Tabungan" : t.type === "debt_payment" ? "Bayar Utang" : "Cicilan", walletMapObj.get(t.wallet_id) || "N/A", categoryMap.get(t.category_id || "") || "Lainnya", formatCurrencyStr(t.amount), t.description || "-"]);
