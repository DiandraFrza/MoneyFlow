...
export const QuickAddModal: React.FC<QuickAddModalProps> = ({ isOpen, onClose }) => {
  ...
  const [date, setDate] = useState<string>('2026-06-18'); // current local date
    ...
    try {
      if (type as any === 'transfer') {
        ...
        await addTransaction(user.id, {
          wallet_id: walletId,
          category_id: type === 'savings' || type === 'debt_payment' ? null : categoryId,
          sub_category_id: type === 'savings' || type === 'debt_payment' ? null : subCategoryId,
          type: type,
          amount: parsedAmount,
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-4 gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
          <button
            ...
            className={`py-2 px-1 rounded-lg text-xs font-bold flex flex-col items-center gap-1 transition-all btn-pressable ${
              type === 'expense' ? 'bg-danger text-white shadow' : 'text-text-mutedLight dark:text-text-mutedDark'
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-1 rounded-lg text-xs font-bold flex flex-col items-center gap-1 transition-all btn-pressable ${
              type === 'income' ? 'bg-success text-white shadow' : 'text-text-mutedLight dark:text-text-mutedDark'
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-1 rounded-lg text-xs font-bold flex flex-col items-center gap-1 transition-all btn-pressable ${
              type as any === 'transfer' ? 'bg-primary text-white shadow' : 'text-text-mutedLight dark:text-text-mutedDark'
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-1 rounded-lg text-xs font-bold flex flex-col items-center gap-1 transition-all btn-pressable ${
              type === 'savings' ? 'bg-amber-500 text-white shadow' : 'text-text-mutedLight dark:text-text-mutedDark'
            }`}
          >
        ...
        <div className="relative">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 font-extrabold text-xl text-text-mutedLight dark:text-text-mutedDark">
            Rp
          </span>
          <Input
            ref={amountRef}
        ...
        <div className="grid grid-cols-4 gap-2">
          {[10000, 20000, 50000, 100000].map((val) => (
            <button
              ...
              className="py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors btn-pressable"
            >
              +{val / 1000}k
            </button>
          ))}
        ...
        <div className="flex flex-col gap-4">
          ...
          {type as any === 'transfer' ? (
            <div className="grid grid-cols-2 gap-3">
              <Select
                ...
                options={activeWallets.map(w => ({ value: w.id, label: `${w.name} (Saldo: Rp${w.balance.toLocaleString('id-ID')})` }))}
              />
              <Select
                label="Ke Dompet"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              type="text"
              label="Catatan"
              placeholder="Makanan, bensin, dll."
              value={description}
          ...
          {(type === 'expense' || type === 'income') && (
            <div className="flex flex-col gap-1.5">
              <span className="text-xs font-semibold text-text-mutedLight dark:text-text-mutedDark tracking-wide">
                Foto Bukti Transaksi (Opsional)
              </span>
                ...
                {receiptUrl && (
                  <div className="relative h-11 w-11 rounded-lg border overflow-hidden">
                    ...
                    <button
                      ...
                      className="absolute inset-0 bg-black/40 text-white flex items-center justify-center font-bold text-[8px]"
                    >
                      Hapus
                    </button>
                  </div>
